...
from pages.emailSectionPage import EmailSectionPage
...
def test_loginPage(page:Page):
    ...
    testEmpUser = config.TEST_EMP_USER
    testBusinessUser = config.TEST_BUSINESS_USER
...
# Search the test employee user
    ...
    userSearchPage.emp_user_search(page,testEmpUser)
...
#  Login as the test business user
    ...
    businessUserLoginPage.business_user_login(page,testBusinessUser)
    
# open newly created IXT record  
    recordDetailPage = RecordDetailPage(page)
    recordDetailPage.record_detail(record_id)
    
# Navigate email section in record and verify Email Status = Sent
    emailSectionPage = EmailSectionPage(page)
    emailSectionPage.verify_email_status(page)
